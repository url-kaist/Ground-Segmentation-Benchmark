...
class odometry:
    ...
    def __init__(self, base_path, sequence, frame_range=None):
        ...
        self.pose_path = os.path.join(base_path, 'poses_gt')
    ...
    def load_poses(self):
        ...
        print('Loading GT poses for sequence ' + self.sequence + '...')
...
if __name__ == "__main__":
    ...
    abs_path = "/media/shapelim/UX960NVMe1/kitti_semantic/dataset/sequences/01"
    output_path = "/media/shapelim/UX960NVMe1/changgi_intensity/01"
    label_path = os.path.join(abs_path, 'labels', '*.label')
    label_files = sorted(glob.glob(label_path))[:10]
    print('Found ' + str(len(label_files)) + ' labels of scans...')

    label = utils.load_labels(label_files)

    velo_path = os.path.join(abs_path, 'velodyne', '*.bin')
    velo_files = sorted(glob.glob(velo_path))[:10]

    velo = utils.load_velo_scans(velo_files)
    print("Done")
    import pandas as pd
    count = 0
    for (label_data, scan_data) in zip(label, velo):
        if (count%10 == 0):
            print("On conducting ", count, "th operation...")
        sem_label = label_data & 0xFFFF
        inst_id = label_data >> 16
        is_traffic = (sem_label == 81)

        xyz = scan_data[:, :3]
        range = np.reshape(np.sqrt(np.sum(np.square(xyz), axis=1)), (-1, 1))
        intensity = scan_data[:, 3:]
        print(range.shape, intensity.shape)

        traffic_inten = np.reshape(intensity[is_traffic], (-1, 1))
        traffic_range = np.reshape(range[is_traffic], (-1, 1))
        traffic_id = np.reshape(inst_id[is_traffic], (-1, 1))
        traffic_info = np.concatenate([traffic_inten, traffic_range, traffic_id], axis=1)
        nontraffic_inten = np.reshape(intensity[np.logical_not(is_traffic)], (-1,1))

        # print(traffic_inten.shape)
        # print(nontraffic_inten.shape)
        df_traffic = pd.DataFrame(traffic_inten)
        df_nontraffic = pd.DataFrame(nontraffic_inten)
        t_fname = output_path + "/traffic_sign/" + str(count).zfill(4) + ".csv"
        nt_fname = output_path + "/nontraffic_sign/" + str(count).zfill(4) + ".csv"
        df_traffic.to_csv(t_fname, index=False)
        df_nontraffic.to_csv(nt_fname, index=False)
