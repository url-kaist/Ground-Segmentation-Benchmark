...
import pykitti
from nonplanar_gpf.msg import node
...
def save_rosbag_via_node(bag, kitti, kitti_type, initial_time):
    ...
    if kitti_type.find("odom") != -1:
        ...
        for timestamp, scan, label, tf_matrix_cam in bar(iterable):
            # 카메라 coordinate (오른쪽)x-(아래쪽)y-(앞쪽)z를 우리가 아는 앞(x)-왼쪽-(y)-위(z)로 바꿔
            tmp = np.matmul(tf_matrix_cam, CAM2LIDAR)
            ...
            fields = [PointField('x', 0, PointField.FLOAT32, 1),
                      ...
                      PointField('id', 20, PointField.UINT16, 1)]
            sem_label = label & 0xFFFF
            inst_id = label >> 16
            # label_float = label.astype(np.float32)
            scan_w_label = np.concatenate((scan, sem_label, inst_id), axis=1)
            ...
            bag.write('/node', out, out.header.stamp)
            # bag.write('/debug/pc_raw', pcl_raw_msg, out.header.stamp)
            count += 1


def save_rosbag_via_pc(bag, kitti, kitti_type, initial_time):
    if kitti_type.find("odom") != -1:
        timestamps = map(lambda x: initial_time + x.total_seconds(), kitti.timestamps)
        assert(len(kitti.T_w_cam0)) == len(kitti.velo)
        assert(len(kitti.velo)) == len(kitti.label)
        print("{} vs {} vs {}. Condition satisfied.".format(len(kitti.T_w_cam0), len(kitti.velo), len(kitti.label)))
        iterable = zip(timestamps, kitti.velo, kitti.label)
        bar = progressbar.ProgressBar()
        count = 0
        for timestamp, scan, label in bar(iterable):
            # 카메라 coordinate (오른쪽)x-(아래쪽)y-(앞쪽)z를 우리가 아는 앞(x)-왼쪽-(y)-위(z)로 바꿔
            header = Header()
            if kitti.frame_range == None:
                header.seq = count
            ...
            bag.write('kitti/velo/pointcloud', pcl_msg, header.stamp)
            # bag.write('/debug/pc_raw', pcl_raw_msg, out.header.stamp)
            count += 1
...
def main():
    ...
    SAVE_PATH = "/media/shapelim/UX960NVMe1/kitti_semantic/pc_only_rosbag_xyzilabel"
    ...
    needs_partial_stamp = False
    if needs_partial_stamp:
        init_stamp = 4390
        final_stamp = 4400
        interval = 1
        ...
        bag = rosbag.Bag(
            os.path.join(args.savedir, "{}_pc_only.bag".format(args.sequence)),
            'w', compression=compression)
        kitti = pykitti.odometry(args.dir, args.sequence, None)
    ...
    try:
        ...
        # save_rosbag_via_node(bag, kitti, args.kitti_type, initial_time=current_epoch)
        # save_rosbag_via_pc(bag, kitti, args.kitti_type, initial_time=current_epoch)
        save_rosbag_via_csv_for_changi(bag, kitti, args.kitti_type, initial_time=current_epoch)
